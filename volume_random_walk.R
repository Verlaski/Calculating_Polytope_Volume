PolytopeVolume <- function(A, B, epsilon){
  ...
  #                  Polytope {x|  (wA)x <= (wB)}, ball {(x-wc)^T(x-wc)<=wr^2}
  #                  ww is repeat times
  # Output Value: point generated 
  we <- matrix(0, nrow = n, ncol = 1)
  wee <- matrix(0, nrow = n, ncol = 1)
  ...
  repeat{
    ...
    # Set as Emin and Emax the min and max eigenvalue of E
    Emax <- Eeign[1]
    Emin <- Eeign[1]
    for(jj in 2:n){
  ...
  while(Fi > alpha){
    ...
    if(N-countprev > 0){
      for(Fj in 1: (N-countprev)){
        p0 <- Walk(p0, TagA, TagB, 1, Center, LargeR, W, m, n)
